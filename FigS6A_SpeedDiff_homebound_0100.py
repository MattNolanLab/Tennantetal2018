...
for dcount,day in enumerate(days):
    for mcount,mouse in enumerate(mice):
        ...
        if mcount == 2 or mcount == 3 or mcount == 9:
            ...
            speeddiff = nbeac[11] - nbeac[17]
            control_nb[mcount,dcount] = speeddiff
            speeddiff = probe[11] - probe[17]
...
ax.tick_params(axis='y', pad = 10, which = 'both', top='off', right = 'off', direction = 'out', length = 8, width = 3, labelsize =5)
plt.locator_params(axis = 'x', nbins  = 2)
plt.locator_params(axis = 'y', nbins  = 4)
ax.axhline(-20,linewidth=3, color="black")
...
plt.savefig('Plots/Supplemental6/Task15_SpeedMeans_homebound_0200' +' .png', dpi = 200)
plt.close()
...
np.savetxt('Data_Output/Supplemental6/Figure4_SpeedDiff_L3-L4_0100.csv', data,fmt = '%s', delimiter = ',', header = 'Genotype fluorescence,Beaconed')
